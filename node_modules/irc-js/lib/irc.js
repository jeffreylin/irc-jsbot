...
function IRC ( options ) {
  ...
  this._internal.emitter.on( 'ping', function( message ){
...
function doConnect () {
  ...
  this._internal.connected_since = +new Date
  ...
  this._internal.privmsg_queue_timer = setInterval( function tick() { var m
    if ( m = this._internal.privmsg_queue.shift() )
...
function doDisconnect () {
  clearInterval( this._internal.privmsg_queue_timer )
  this._internal.privmsg_queue = []
...
function parseMessage ( data ) { var buffer, last, message, command, i
  this._internal.last_message = +new Date
  ...
  for ( i = 0; i < buffer.length; i++ ) {
    ...
    try {
      ...
      console.error( "[ERROR] Failed parsing '" + buffer[i] + "'" )
      if ( this.options.die )
        throw err
    }
    ...
    else if ( command == 'nick' && message.person.nick == this._internal.nick )
      this._internal.nick = message.params[0]
 ...
 *
 * Send a raw IRC command to the server. Used this._internally by the convenience
...
IRC.prototype.raw = function raw ( data ) {
  if ( this._internal.socket.readyState == 'open' ) {
    ...
    if ( !/\r\n$/.test( data ) )
      data += "\r\n"
    this._internal.socket.write( data )
    if ( this.options.log )
      console.info( "[SENT]  " + data.replace( /\r\n$/, "" ) )
  }
  return this
}
...
IRC.prototype.on = function on ( event, hollaback ) {
  ...
  if ( !this._internal.listener_cache[event] )
    this._internal.listener_cache[event] = []

  this._internal.listener_cache[event].push( bound )
...
IRC.prototype.removeListener = function removeListener ( event, hollaback ) { var eventCache, i
  if ( this._internal.listener_cache[event] ) {
    eventCache = this._internal.listener_cache[event]
    for ( i = 0; i < eventCache.length; i++ ) {
      if ( eventCache[i] === hollaback ) {
        ...
        this._internal.emitter.removeListener( event, eventCache[i] )
      }
    }
  }
  return this
...
IRC.prototype.nick = function nick ( nickname ) {
  // 4.1.2
  this.raw( ( this._internal.nick === undefined ? '' : ':' + this._internal.nick + ' ' ) + 'NICK' + param( nickname ) )
  return this
}
...
IRC.prototype.userMode = function userMode ( mode ) {
  // 4.2.3.2
  return this.raw( param( this._internal.nick, null, ':' ) + ' MODE' + param( mode ) )
}
...
IRC.prototype.topic = function topic ( channel, type ) {
  ...
  if ( typeof type === 'function' ) {
    ...
    this.once( '332', function( message ) {
      type.call( this, message.params[1], message.params[ message.params.length - 1 ] )
    })
...
IRC.prototype.list = function list ( channel, hollaback ) {
  ...
  this.raw( 'LIST' + if_exists( channel ) )
  return this
}
...
function gatherList () {
  ...
  this._internal.emitter.on( '322', gatherChannels )
  this._internal.emitter.on( '323', function() {
    this._internal.emitter.removeListener( '322', gatherChannels )
...
IRC.prototype.privmsg = function privmsg ( receiver, msg, protect ) { var private_messages, i
  ...
  for ( i = 0; i < private_messages.length; i++ )
    if ( protect && i > 0 )
      this._internal.privmsg_queue.push({ receiver: receiver, message: private_messages[i] })
    else
